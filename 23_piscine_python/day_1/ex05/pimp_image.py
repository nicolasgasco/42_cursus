...
def _describe_image(image: numpy.ndarray) -> None:
    """
    Prints the shape and content of the given image.

    Parameters:
    image (numpy.ndarray): The image to describe.
...
def ft_grey(array) -> None:
    ...
    for y in range(len(array)):
        for x in range(len(array[y])):
            ...
            new_value = red_value / 3.0 + green_value / 3.0 + blue_value / 3.0
            new_image[y, x] = [new_value, new_value, new_value]
