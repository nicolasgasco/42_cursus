...
int main(void)
{
    ...
    std::vector<int>::iterator ite = myList.end();
    for (; it != ite; ++it)
    ...
    std::vector<float> myListF;
    srand(time(0));
    for (int i = 0; i < 10; ++i)
        myListF.push_back(static_cast<float>(rand()) / (static_cast<float>(RAND_MAX)));

    std::cout << "Printing list using iterators" << std::endl;
    std::vector<float>::iterator itf = myListF.begin();
    std::vector<float>::iterator itef = myListF.end();
    for (; itf != itef; ++itf)
    ...
    std::vector<char> myListC;
    srand(time(0));
    for (int i = 0; i < 10; ++i)
        myListC.push_back('a' + rand() % 26);

    std::cout << "Printing list using iterators" << std::endl;
    std::vector<char>::iterator itc = myListC.begin();
    std::vector<char>::iterator itec = myListC.end();
    for (; itc != itec; ++itc)
