...
|**minitalk**|C/UNIX|October 2021|Code a small data exchange program using UNIX signals (SIGUSR1 and SIGUSR2) and bitwise operations. A string is passed as parameter to the client, converted into binary, and sent to the server in form of signals (either 0 or 1), where the string is reconstructed and outputted.|
...
|**ft_printf**|C|September 2021|Reproduce basic functioning of prinft function (flags implemented: %c, %s, %i, %d, %u, %x, %X, %p, %%, no error handling except for NULL strings passed as parameter)|
...
|**libft**|C|July 2021|Re-write useful C functions like split or join using only malloc and free|
