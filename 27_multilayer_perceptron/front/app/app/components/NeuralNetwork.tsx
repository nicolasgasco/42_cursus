    ...
    <div className="flex gap-4">
            ...
            {layer.weights.map((weight, weightIndex) => {
              return (
                <NeuronTile key={weightIndex}>
                  ...
                  <p>Weights: [{weight.map(weight => weight.toFixed(6)).join(", ")}]</p>
      ...
      <LayerTile title={"Predictions"}>
        <table className="table-auto m-4">
          <thead>
            <tr>
              <th className="px-6 pb-2 border-b-2">Malignant</th>
              <th className="px-6 pb-2 border-b-2">Benign</th>
            </tr>
          </thead>
          <tbody>
                  ...
                  {Object.values(value).map((value, valueIndex) => {
                    return (
                      <td key={valueIndex} className={`px-6 pb-1 ${index === 0 ? 'pt-4' : ''}`}>
                        <span className={value > 0.5 ? "text-yellow-300" : "font-light"}>{value.toFixed(6)}</span>
                      </td>
                    )
                  })}
                </tr>
