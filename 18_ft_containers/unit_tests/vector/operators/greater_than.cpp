...
void greaterThanUnitTests(bool isDebug)
{
    outputSuiteTitle("GREATER THAN:");
    {
        ...
        }
        {
            ...
            std::vector<int> seedOriginal;
            std::vector<int> original;
            outputAssertion("returns false when size = 0", isStrictEqual<bool>((own > seedOwn), (original > seedOriginal), isDebug));
        }
        {
        ...
        }
        {
            ...
            ft::vector<int> seedOwn;
            fillVectorWithValues<int>(seedOwn, values1, 4);
            ft::vector<int> own;
            fillVectorWithValues<int>(own, values2, 4);

            std::vector<int> seedOriginal;
            fillVectorWithValues<int>(seedOriginal, values1, 4);
            std::vector<int> original;
            fillVectorWithValues<int>(original, values2, 4);
            outputAssertion("returns true when same size and v1 > v2", isStrictEqual<bool>((own > seedOwn), (original > seedOriginal), isDebug));
        }
        {
