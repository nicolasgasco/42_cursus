...
/*   Updated: 2021/08/26 10:15:46 by ngasco           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	char		*line;
	int			b_read;
	static char	*buf_static[10000000];
	...
	while (ft_find_nl(buf_static[fd]) == -1)
	{
		...
		if (b_read <= 0)
			break ;
		buffer[b_read] = '\0';
		buf_static[fd] = ft_strjoin(buf_static[fd], buffer);
	}
	free(buffer);
	if (b_read == -1)
		return (NULL);
	line = ft_calc_line(buf_static[fd]);
	buf_static[fd] = ft_calc_buf(buf_static[fd]);
	return (line);
}
